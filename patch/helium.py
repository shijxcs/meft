...
from .functional import helium_rms_norm_forward, nn_linear_forward, llama_mlp_forward


def apply_patch_to_helium_model(
    model: "transformers.models.helium.modeling_helium.HeliumPreTrainedModel",
    norm: bool = False,
    attn_in: bool = False,
    attn_out: bool = False,
    mlp_in: bool = False,
    ...
    from transformers.models.helium.modeling_helium import HeliumModel, HeliumDecoderLayer
    base_model: HeliumModel = model.base_model

    for layer in base_model.layers:
        layer: HeliumDecoderLayer
        if norm:
            _patch_module(layer.input_layernorm, helium_rms_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.post_attention_layernorm, helium_rms_norm_forward, compress_kwargs=compress_kwargs)
        if attn_in:
