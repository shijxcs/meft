...
from .functional import gemma_rms_norm_forward, nn_linear_forward, llama_mlp_forward


def apply_patch_to_gemma3_model(
    model: "transformers.models.gemma3.modeling_gemma3.Gemma3PreTrainedModel",
    norm: bool = False,
    attn_in: bool = False,
    attn_out: bool = False,
    mlp_in: bool = False,
    ...
    base_model: Gemma3Model = model.base_model

    apply_patch_to_gemma3_text_model(
...
def apply_patch_to_gemma3_text_model(
    model: "transformers.models.gemma3.modeling_gemma3.Gemma3PreTrainedModel",
    norm: bool = False,
    attn_in: bool = False,
    attn_out: bool = False,
    ...
    base_model: Gemma3TextModel = model.base_model

    for layer in base_model.layers:
        layer: Gemma3DecoderLayer
        if norm:
            _patch_module(layer.input_layernorm, gemma_rms_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.post_attention_layernorm, gemma_rms_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.pre_feedforward_layernorm, gemma_rms_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.post_feedforward_layernorm, gemma_rms_norm_forward, compress_kwargs=compress_kwargs)
        if attn_in:
