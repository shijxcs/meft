...
from .functional import olmo_layer_norm_forward, nn_linear_forward, llama_mlp_forward


def apply_patch_to_olmo_model(
    model: "transformers.models.olmo.modeling_olmo.OlmoPreTrainedModel",
    norm: bool = False,
    attn_in: bool = False,
    attn_out: bool = False,
    mlp_in: bool = False,
    ...
    base_model: OlmoModel = model.base_model

    for layer in base_model.layers:
        layer: OlmoDecoderLayer
        if norm:
            _patch_module(layer.input_layernorm, olmo_layer_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.post_attention_layernorm, olmo_layer_norm_forward, compress_kwargs=compress_kwargs)
        if attn_in:
