...
from ..functions import t5_rms_norm_forward, nn_linear_forward, llama_mlp_forward
from ..patch import _checkpoint_module, _patch_module


def apply_patch_to_olmo2_model(
    model: "transformers.models.olmo2.modeling_olmo2.Olmo2PreTrainedModel",
    norm: bool = False,
    attn_in: bool = False,
    attn_out: bool = False,
    mlp_in: bool = False,
    ...
    from transformers.models.olmo2.modeling_olmo2 import Olmo2Model, Olmo2DecoderLayer
    base_model: Olmo2Model = model.base_model

    for layer in base_model.layers:
        layer: Olmo2DecoderLayer
        if norm:
            _patch_module(layer.post_attention_layernorm, t5_rms_norm_forward, compress_kwargs=compress_kwargs)
            _patch_module(layer.post_feedforward_layernorm, t5_rms_norm_forward, compress_kwargs=compress_kwargs)
        if attn_in:
