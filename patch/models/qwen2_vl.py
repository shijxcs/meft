...
from ..functions import t5_rms_norm_forward, nn_layer_norm_forward, nn_linear_forward, llama_mlp_forward, quick_gelu_forward
...
def apply_patch_to_qwen2_vl_text_model(
    ...
    from transformers.models.qwen2_vl.modeling_qwen2_vl import Qwen2VLTextModel, Qwen2VLDecoderLayer, Qwen2VLAttention
    base_model: Qwen2VLTextModel = model.base_model

    for layer in base_model.layers:
        layer: Qwen2VLDecoderLayer
        self_attn: Qwen2VLAttention = layer.self_attn
        if norm:
...
def apply_patch_to_qwen2_vl_vision_model(
    ...
    from transformers.models.qwen2_vl.modeling_qwen2_vl import Qwen2VisionTransformerPretrainedModel, Qwen2VLVisionBlock, VisionAttention
    base_model: Qwen2VisionTransformerPretrainedModel = model.base_model

    for layer in base_model.blocks:
        layer: Qwen2VLVisionBlock
        attn: VisionAttention = layer.attn
        if norm:
